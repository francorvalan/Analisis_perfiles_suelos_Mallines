...
Se analizaron `r nrow(horizons) ` capas de suelos, localizadas en  `r length(horizons)` perfiles, comprendiendo distintas profundidades desde la superfie hasta `r max(horizons$bottom,na.rm=T)/100` m. 

De los mismos, `r checkHzDepthLogic(horizons)%>% filter(valid==F) %>% nrow()` perfiles presentaron problemas en sus profundidades, los cuales se detallan a continuación
...
kable(horizons_B %>% filter(Mallin %in% (horizons_checked%>% filter(valid==F) %>% pull(Mallin))) %>% select(Mallin,top,bottom,SAND,SILT,CLAY), align = "c",caption = "Capas de suelo con problemas en profundidades",digits=1)
...
# graphical check
par(mar = c(1,1,1,1))
Output_dir <- "./03_Results/01_Profile_plots"
if(!dir.exists(Output_dir))dir.create(Output_dir)

for(i in 1:length(horizons_cleaned)){

  jpeg(file=paste0(Output_dir,"/Profile_",unique(horizons_cleaned[i,]$Mallin),".jpeg"),width=600, height=750, quality = 200)
  plotSPC(unique(horizons_cleaned[i,], vars = c('top', 'bottom')), 
                name.style = 'center-center', width = 0.15,color="horizons_munsell_color", 
                fixLabelCollisions = TRUE,plot.depth.axis = T,axis.line.offset = -10,
                hz.depths = TRUE,hz.depths.lines=F,hz.depths.offset = c(0.01),shrink=T, cex.names=1.5)
  dev.off()
  # plotSPC(horizons_cleaned[i,], cex.names = 1, name.style = 'center-center', width = 0.3, plot.depth.axis = T, hz.depths = TRUE,
  #         hz.depths.lines=T, fixLabelCollisions = TRUE,   hz.depths.offset = c(0.005),
  #         color="horizons_munsell_color")
...
```
```{r echo=F, warning=F, message=F}
Triangle_color_wout_legend <- ggplot(data = USDA, aes(
  y = Arcilla,
  x = Arena,
  z = Limo
)) +
...
Triangle_color_wout_legend

```

```{r echo=F , warning=F, message=F}
theme_set(theme_bw())
Triangle_white_with_legend <- ggplot(data = USDA, aes(
  y = Arcilla,
  x = Arena,
  z = Limo
)) +
...
Triangle_white_with_legend

```

```{r echo=F , warning=F, message=F}
theme_set(theme_bw())
Triangle_white_wout_legend <- ggplot(data = USDA, aes(
  y = Arcilla,
  x = Arena,
  z = Limo
)) +
  ...
  theme(text = element_text(family = "Helvetica")) +
   labs(fill = "Referencias") 
  #guides(fill=T, color=FALSE)
Triangle_white_wout_legend

```

```{r include=T, echo=F, message=F, warning=FALSE,results=F}
# graphical check
par(mar = c(1,1,1,1))
Output_dir <- "./03_Results/02_Triangle_texture_plots"
if(!dir.exists(Output_dir))dir.create(Output_dir)

jpeg(file=paste0(Output_dir,"/Triangle_white_wout_legend.jpeg"),width=1200, height=1200, quality = 100,res = 250)
Triangle_white_wout_legend
dev.off()

jpeg(file=paste0(Output_dir,"/Triangle_white_with_legend.jpeg"),width=1200, height=1200, quality = 100,res = 150)
Triangle_white_with_legend
dev.off()

jpeg(file=paste0(Output_dir,"/Triangle_color_wout_legend.jpeg"),width=1200, height=1200, quality = 100,res = 250)
Triangle_color_wout_legend
dev.off()

jpeg(file=paste0(Output_dir,"/Triangle_color_with_legend.jpeg"),width=1200, height=1200, quality = 100,res = 150)
Triangle_color_with_legend
dev.off()
